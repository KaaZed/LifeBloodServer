...
SPEED_MAX_KMH = 8.0
LBC_PER_STEP = 0.00043             # начисление за шаг пользователю
    ...
    async def process_location(
        ...
        *,
        ...
        async with self.pool.acquire() as c, c.transaction():
            ...
            st = await c.fetchrow("SELECT cluster_lat, cluster_lon, last_ts FROM user_state WHERE user_id=$1", user_id)
            if not st or st["cluster_lat"] is None or st["cluster_lon"] is None:
                # Инициализация пятна
                await c.execute("""
                    INSERT INTO user_state (user_id, cluster_lat, cluster_lon, last_ts)
                    VALUES ($1,$2,$3,$4)
            ...
            cl_lat, cl_lon, last_ts = st["cluster_lat"], st["cluster_lon"], st["last_ts"] or t
            dist_m = _haversine_m(cl_lat, cl_lon, lat, lon)

            # Пока не «набежало» 100 м — просто обновляем last_ts и причину
            if dist_m < CLUSTER_M:
                await c.execute("""
                    UPDATE user_state SET last_ts=$2 WHERE user_id=$1
                """, user_id, t)
                await c.execute("UPDATE users SET reason_if_not_counted=$2, updated_at=now() WHERE user_id=$1",
            ...
            # Проверка скорости
            dt_sec = max(1.0, (t - (last_ts or t)).total_seconds())
            ...
            if speed_kmh < SPEED_MIN_KMH or speed_kmh > SPEED_MAX_KMH:
                ...
                await c.execute("""
                    UPDATE user_state SET cluster_lat=$2, cluster_lon=$3, last_ts=$4 WHERE user_id=$1
                """, user_id, lat, lon, t)
                await c.execute("UPDATE users SET reason_if_not_counted=$2, updated_at=now() WHERE user_id=$1",
            ...
            if steps <= 0:
                ...
                await c.execute("UPDATE user_state SET cluster_lat=$2, cluster_lon=$3, last_ts=$4 WHERE user_id=$1",
                                user_id, lat, lon, t)
...
async def _lb_bridge_process_location(self,
                                      user_id: int,
                                      chat_id: int,
                                      lat: float,
                                      lon: float,
    ...
    """
    Compatibility bridge for webhook_app.py:
      process_location(user_id, chat_id, lat, lon, *, ts=None, accuracy=None, heading=None, speed=None)
    Tries to delegate to existing implementations; otherwise returns a safe fallback.
    """
    # 1) Try to delegate to richer/newer implementations if present
    candidates = [
        getattr(self, "process_location_v2", None),
        getattr(self, "_process_location_v2", None),
        getattr(self, "process_location_core", None),
        getattr(self, "_process_location_core", None),
        # last resort: an existing process_location with legacy signature
        getattr(self, "process_location", None),
        getattr(self, "_process_location", None),
    ]

    for meth in candidates:
        if meth is None:
            continue
        # Avoid infinite recursion if we point to ourselves
        if getattr(meth, "__name__", "") == "_lb_bridge_process_location":
            continue

        try:
            # Try rich kwargs first
            return await meth(user_id, lat, lon, ts=ts, accuracy=accuracy, heading=heading, speed=speed)
        except TypeError:
            # Fallback to the simplest legacy form
            try:
                return await meth(user_id, lat, lon)
            except TypeError:
                # Different shape — try a couple more common variants
                try:
                    return await meth(user_id, lat, lon, ts)
                except TypeError:
                    pass
        except Exception as e:
            # Swallow and try next candidate
            try:
                log = getattr(self, "log", None) or getattr(self, "logger", None)
                if log and hasattr(log, "error"):
                    log.error(f"[bridge] delegate error: {e}")
            except Exception:
                pass

    # 2) Minimal safe fallback: try to persist raw point if pool exists
    try:
        if hasattr(self, "pool") and self.pool:
            async with self.pool.acquire() as c:
                ...
                await c.execute(
                    "INSERT INTO raw_points(user_id, lat, lon, ts, accuracy, heading, speed) "
                    "VALUES($1,$2,$3, COALESCE(to_timestamp($4), now()), $5, $6, $7)",
                    user_id, lat, lon, ts, accuracy, heading, speed
                )
    except Exception as e:
        try:
            log = getattr(self, "log", None) or getattr(self, "logger", None)
            if log and hasattr(log, "error"):
                log.error(f"[bridge] raw insert failed: {e}")
        except Exception:
            pass

    # Return structure that webhook expects not to crash on
    return {"ok": True, "counted": 0, "reason": "bridge"}

# Bind the bridge as the public API
try:
    LifeBloodDB.process_location = _lb_bridge_process_location  # type: ignore[name-defined]
except NameError:
    # If class name differs, do nothing (won't happen in our project)
    pass
# --- LB_BRIDGE_111 end ---


# === BEGIN unified process_location adapter v2 (name-agnostic) ===
from typing import Any

async def _lb_call_pl_candidates(self, user_id, username, lat, lon, ts, names):
    # Пробуем несколько сигнатур подряд, не падаем на TypeError
    for name in names:
        fn = getattr(self, name, None)
        if not callable(fn):
            continue
        try:
            # Новый стиль: (user_id, username, lat, lon, *, ts=...)
            return await fn(user_id, username, float(lat), float(lon), ts=ts)
        except TypeError:
            pass
        try:
            # Полустарый: (user_id, lat, lon, ts)
            return await fn(user_id, float(lat), float(lon), ts)
        except TypeError:
            pass
        try:
            # Старый: (user_id, lat, lon)
            return await fn(user_id, float(lat), float(lon))
        except TypeError:
            continue
        except Exception:
            # Любая иная ошибка реализации — прекращаем попытки
            break
    # Фоллбек: ничего не начисляем, но и не роняем webhook
    return {"added_steps": 0, "added_lbc": 0.0, "speed_kmh": 0.0, "dist_m": 0.0, "reason": "ok"}

def _install_unified_process_location_adapter():
    import inspect
    candidates = [
        "_lb_bridge_process_location",
        "process_location_v2",
        "_process_location_v2",
        "process_location_old",
        "_process_location_old",
    ]

    def _mk_adapter():
        async def process_location(self, user_id: int, username: str, *, ts=None, lat=None, lon=None,
                           accuracy=None, heading=None, live_period=None, chat_id=None):
    """
    Сохраняем точку, считаем шаги и обновляем user_state.
    ВАЖНО: совместимость со старыми схемами БД.
      - Если locations.ts = TIMESTAMPTZ → пишем datetime
      - Если locations.ts = numeric/int/bigint → пишем epoch (int)
    """
    from datetime import datetime, timezone
    from math import radians, sin, cos, sqrt, atan2

    async def _ensure_user(conn):
        await conn.execute(
        ...
        await conn.execute(
            """
            INSERT INTO user_state(user_id, cluster_lat, cluster_lon, last_ts, daily_steps, daily_charge, steps_total, dash_msg_id)
            VALUES ($1, NULL, NULL, NULL, 0, 0, 0, NULL)
            ON CONFLICT (user_id) DO NOTHING
            """, user_id
        )

    async def _loc_ts_mode(conn):
        # Кэшируем определение типа
        if getattr(self, "_loc_ts_mode_cache", None) in ("ts", "epoch"):
            return self._loc_ts_mode_cache
        row = await conn.fetchrow("""
            SELECT data_type, udt_name
            FROM information_schema.columns
            WHERE table_schema='public' AND table_name='locations' AND column_name='ts'
        """)
        mode = "ts"
        if row:
            dt = (row.get("data_type") or "").lower()
            udt = (row.get("udt_name") or "").lower()
            if ("timestamp" in dt) or ("timestamptz" in dt):
                mode = "ts"
            else:
                mode = "epoch"
        self._loc_ts_mode_cache = mode
        return mode
    ...
    try:
        ...
        ts_num = int(datetime.now(tz=timezone.utc).timestamp())

    async with self.pool.acquire() as conn:
        await _ensure_user(conn)

        # Определяем режим записи ts
        mode = await _loc_ts_mode(conn)
        # Значение для last_ts (в user_state) всегда datetime
        t_dt = datetime.fromtimestamp(ts_num, tz=timezone.utc)

        # читаем предыдущее состояние
        st = await conn.fetchrow("SELECT cluster_lat, cluster_lon, last_ts, daily_steps, steps_total FROM user_state WHERE user_id=$1", user_id)
        clat = st["cluster_lat"] if st else None
        clon = st["cluster_lon"] if st else None
        last_ts = st["last_ts"] if st else None

        # 3.1 Сохраняем точки локации (устойчиво к типу ts)
        if mode == "ts":
            await conn.execute(
                "INSERT INTO locations (user_id, ts, lat, lon, accuracy, heading) VALUES ($1,$2,$3,$4,$5,$6)",
                user_id, t_dt, lat, lon, accuracy, heading
            )
        else:
            await conn.execute(
                "INSERT INTO locations (user_id, ts, lat, lon, accuracy, heading) VALUES ($1,$2,$3,$4,$5,$6)",
                user_id, ts_num, lat, lon, accuracy, heading
            )

        # 3.2 Пересчет метрик/кластер
        # если нет кластера — инициализируем
        if clat is None or clon is None:
            await conn.execute(
                "UPDATE user_state SET cluster_lat=$2, cluster_lon=$3, last_ts=$4 WHERE user_id=$1",
                user_id, lat, lon, t_dt
            )
            return

        # расстояние и скорость
        dist_m = _haversine_m(clat, clon, float(lat), float(lon))
        dt_s = max(1, int(t_dt.timestamp() - (last_ts.timestamp() if last_ts else t_dt.timestamp())))
        speed_kmh = (dist_m / dt_s) * 3.6

        # фильтры (диапазоны — из WP: пешеход 3-8 км/ч)
        if speed_kmh < 3.0 or speed_kmh > 8.0 or dist_m < 5.0:
            # Просто обновим last_ts и точку кластера с очень мягким дрейфом
            new_lat = (clat*0.95 + lat*0.05)
            new_lon = (clon*0.95 + lon*0.05)
            await conn.execute(
                "UPDATE user_state SET cluster_lat=$2, cluster_lon=$3, last_ts=$4 WHERE user_id=$1",
                user_id, new_lat, new_lon, t_dt
            )
            return

        # конверсия расстояния -> шаги (0.75 м/шаг)
        steps = int(dist_m / 0.75)
        if steps < 1:
            await conn.execute("UPDATE user_state SET last_ts=$2 WHERE user_id=$1", user_id, t_dt)
            return

        # Накапливаем шаги и слегка сдвигаем «центр»
        new_lat = (clat*0.7 + lat*0.3)
        new_lon = (clon*0.7 + lon*0.3)
        await conn.execute(
            """
            UPDATE user_state
            SET cluster_lat=$2, cluster_lon=$3, last_ts=$4,
                daily_steps = COALESCE(daily_steps,0) + $5,
                steps_total = COALESCE(steps_total,0) + $5
            WHERE user_id=$1
            """,
            user_id, new_lat, new_lon, t_dt, steps
        )
